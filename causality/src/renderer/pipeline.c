...
static const char *FRAG_GLSL =
    ...
    "    vec2  clip_size;\n"
    "    float clip_radius;\n"
    "    float edge_aa_scale;  /* physical px per logical px (content_scale) */\n"
    "} clip_pc;\n"
    "\n"
    "#define MODE_GLOW        4u\n"
    ...
    "    if (v_mode == MODE_GLOW) {\n"
    "        float radius = max(v_blur, 1e-4);\n"
    "        vec2 source_half = max(box_half - vec2(radius), vec2(0.0));\n"
    "        float d = max(roundedBoxSDF(p, source_half, r), 0.0);\n"
    "        float t = clamp(d / radius, 0.0, 1.0);\n"
    "        float halo = (exp(-4.5 * t * t) - exp(-4.5)) / (1.0 - exp(-4.5));\n"
    ...
    "       p/d_outer are in logical-pixel node space (see v_local in\n"
    "       VERT_GLSL), but rasterisation happens once per *physical*\n"
    "       fragment. A derivative-based ramp (fwidth(d_outer)) measures\n"
    "       logical-unit change per physical fragment, which shrinks by\n"
    "       exactly 1/edge_aa_scale at any DPI above 1x -- the ramp collapses\n"
    "       well below a physical pixel and edges look jagged instead of\n"
    "       smooth. A fixed logical-space half-width, scaled by\n"
    "       1/edge_aa_scale, keeps the ramp a constant physical width at any\n"
    "       content_scale (matching the clip_pc.edge_aa_scale this batch was\n"
    "       pushed with in swapchain.c). 1.75px rather than exactly 1px: a\n"
    "       single-physical-pixel ramp still reads as faceted/staircased on\n"
    "       curved edges (rounded corners) even though it is technically\n"
    "       antialiased -- the extra width softens the curve without a\n"
    "       visibly blurry edge. ---- */\n"
    "    float aa_hw    = 0.875 / max(clip_pc.edge_aa_scale, 1e-4);\n"
    "    float d_outer  = roundedBoxSDF(p, box_half, r);\n"
    "    float aa_outer = 1.0 - smoothstep(-aa_hw, aa_hw, d_outer);\n"
    ...
    "        float aa_inner  = 1.0 - smoothstep(-aa_hw, aa_hw, d_inner);\n"
...
static const char *IMAGE_VERT_GLSL =
    ...
    "    vec2 edge_aa_scale_pad; // .x = physical px per logical px\n"
    "    vec2 _pad1[4];\n"
    "};\n"
    "\n"
    "layout(std430, set = 0, binding = 0) readonly buffer SSB {\n"
    ...
    "layout(location = 5) out float v_edge_aa_scale;\n"
    "\n"
    "void main() {\n"
...
static const char *IMAGE_FRAG_GLSL =
    ...
    "layout(location = 5) in  float v_edge_aa_scale;\n"
    "layout(location = 0) out vec4 out_color;\n"
    "\n"
    "vec3 srgb_to_linear(vec3 c) {\n"
    "    bvec3 cutoff = lessThan(c, vec3(0.04045));\n"
    ...
    "\n"
    "void main() {\n"
    "    /* Fixed logical-space AA half-width scaled to a constant physical\n"
    "       width (1.75px, matching FRAG_GLSL's rect/border ramp -- see its\n"
    "       comment for why fwidth() is wrong here at any content_scale above\n"
    "       1x, and why 1.75px rather than exactly 1px). */\n"
    ...
    "                                   v_corner_radii);\n"
    "    float coverage = 1.0 - smoothstep(-aa_hw, aa_hw, distance);\n"
