...
static const char *FRAG_GLSL =
    ...
    "       1/edge_aa_scale, keeps the ramp exactly one physical pixel wide\n"
    "       at any content_scale (matching the clip_pc.edge_aa_scale this\n"
    "       batch was pushed with in swapchain.c). ---- */\n"
    "    float aa_hw    = 0.5 / max(clip_pc.edge_aa_scale, 1e-4);\n"
...
static const char *IMAGE_FRAG_GLSL =
    ...
    "\n"
    "void main() {\n"
    "    /* Fixed logical-space AA half-width scaled to one physical pixel;\n"
    "       see the matching comment in FRAG_GLSL for why fwidth() is wrong\n"
    "       here at any content_scale above 1x. */\n"
    "    float aa_hw    = 0.5 / max(v_edge_aa_scale, 1e-4);\n"
