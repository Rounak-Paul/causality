...
{
    pthread_mutex_init(&inst->event_mutex, NULL);
    inst->event_head = 0;
    inst->event_tail = 0;
}
...
{
    pthread_mutex_destroy(&inst->event_mutex);
}

void ca_event_post(Ca_Instance *inst, const Ca_Event *event)
{
    pthread_mutex_lock(&inst->event_mutex);
    ...
    pthread_mutex_unlock(&inst->event_mutex);
}
